...
サ終に伴い、2026 年 2 月いっぱいでプロジェクトの新規作成や編集ができなくなってしなった 8thwall ですが、Distributed Engine Binary に引き続き、OSS 版のエンジンコードも公開されましたね。
...
いろいろ試したところ、Docker 上で 8thwall のエンジンコードを JS バイナリにビルドできたので、その方法をご紹介します。
...
その翌月、同ブログにて「Distributed Engine Binary」と「Open Source Components」を準備中ということが明らかにされました。
...
全て見られたわけではないので正確ではないかもですが、推測するに 8thwall ではコアなロジックは C++によって記述されており、それが Emscripten によって WebAssembly にビルドされて JS バイナリに含まれているのだと考えます。[`/c8`](https://github.com/8thwall/8thwall/tree/main/c8)というディレクトリを見ると機能ごとに C++のコードが入っているのがわかりますね。
<!-- textlint-enable -->
...
色々試してみたところ、必要な依存関係をインストールした Ubuntu ベースの Docker イメージを作ってしまったほうが良さそうでしたので、そうしています。
...
3. 出力されたバイナリをマウントしたディレクトリにコピーして取り出す
...
お使いの環境の通信速度などにもよりますが弊環境では 20 分以上かかりました。
途中で Huggingface から TF Lite 用のモデルデータ（なのかな）をバンバンフェッチしていたりしていて、結構通信帯域を食うんですよね。そしてこれらはローカルファイルに展開されるので、ストレージ容量も持っていかれます（docker のディスクファイルが膨れます）。実行する際には余裕をもって 40GB くらいは空けておくことをおすすめします。
