...
具体的には「SignalR」と「Azure Kinect」を扱います。
...
この仕組みはインポート・エクスポートが容易であるため
Unity 開発者であれば一度は使ったことがあるのではないでしょうか。
...
名前やバージョン情報がテキストとして記録され依存パッケージは lock されます。
...
`Packages/manifest.json`と`Packages/packages-lock.json`が生成され、
...
パッケージは必ず Assembly Definition が切られている決まりがあるので、
大量にパッケージをインポートする場合、UPM を使ったときには 2 回目以降のビルド時間を短くできる可能性があります。
...
.NET 環境で動作するパッケージはマネージド DLL として
NuGet のレジストリにアップロードされ、
...
.NET Standard 2.0 対応の DLL のみがインポートされます。
...
Azure Kinect を動かすためには`k4a.dll`と`depthengine_2_0.dll`
という DLL が必要ですが、
これらは.NET Standard 2.0 用のディレクトリに
含まれておらずインポートできないのです。
