...
そして`vite.config.dts`にプラグインの設定を追記します。ライブラリのエントリポイントが変わったことにより、Vite の lib モードで出力する際の設定も変わっていますので合わせて確認しましょう。
...
npm パッケージを公開する際には、`package.json`へいくつか設定を追加する必要があります。Vite の library テンプレートでは既に moin, module, types フィールドなどの設定が追加されていますが、今回は型定義ファイルの場所や出力される JS ファイルの名前が変わっているので修正しましょう。
