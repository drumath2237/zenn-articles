...
2 つ目は今回執筆フローで導入した
GitHub Oriented Writing という考え方をご紹介します。
...
技術者からしてみればたたき売りしてるみたいな金額に見えますよね。
...
書典運営のサポート体制ということでしたら、
「あとから印刷」を語らずにはいられません。
同人誌は電子本と物理本の二種類を頒布できますが、
物理本は電子版よりも制作コストが高めです。
...
執筆では Re:VIEW という言語を使用していまして、
Re:VIEW の説明を公式サイトから引用すると次のようになります。
...
こうすることでビルドが走り、
`/articles`ディレクトリに PDF ファイルが作成されます。
...
章が完成していなくても PR 作って push していました。
...
本来はフランクに質問をしたり、開発者がアイデアについて意見を集めたりする機能ですが、
個人で使うにも結構使えたりします。
