...
本記事では、Unity を使用して Geosspatial API でのデバッグを ARCore Recording and Playback API と併用し、
...
現実空間と Unity のワールド空間の位置合わせを行います。
...
記録したセッションを再生するには、次のような処理を実装します。
