...
本記事では次のような読者像を想定していますが、必ずしも当てはまる必要はありません。
...
`export_name`は`no_mangle`でも構いませんし、`f64`を`f32`にしていますが特に意味はありません。
...
執筆時点（2026 年 7 月）では、これで Vite 8.1.5 が使われるようでした。もし 8.1.0 よりも低いバージョンがインストールされたらバージョンを更新してください。
...
Vite 8.1 でサポートされた Wasm ESM Integration についてご紹介しました。
