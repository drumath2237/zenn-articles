...
実機で動いているアプリケーションに対してもデバッガ同様、
Unity プロセスをアタッチして使うことができます。
このツールも同一ネットワーク上にあれば、IP アドレスを指定して使えるので
adb 接続は不要です。

それでは実際に使って見ましょう。
Profiler を使うためには
「ウィンドウ」→「Analytics」→「Unity Profiler」でプロファイラウィンドウが起動します。
Development Build されたアプリをデバイスで起動し、
デバイスの IP アドレスをプロファイラに指定します。
アドレスが間違っていなければ自動でパフォーマンスの解析が始まるはずです。

使うとわかるのですが、
`Debug.Log()`で指定したログがエディタ上で出力されているのが分かります。
Profiler を使うと Android Logcat を使わなくても Debug.Log を見ることが出居るので便利です。
